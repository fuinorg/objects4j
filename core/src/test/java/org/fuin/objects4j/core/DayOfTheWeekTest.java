...
class DayOfTheWeekTest {
    ...
    void testFollows() {
        ...
        for (final DayOfTheWeek dow : DayOfTheWeek.getAll()) {
            if (last != null && dow != DayOfTheWeek.PH) {
                assertThat(dow.follows(last)).describedAs("Expected that " + dow + " follows on " + last + ", but did not").isTrue();
            }
        ...
        for (final DayOfTheWeek dow : DayOfTheWeek.getAll()) {
            if (current != null) {
                for (final DayOfTheWeek dow2 : DayOfTheWeek.getPart(dow, DayOfTheWeek.SUN)) {
                    assertThat(dow2.after(current)).describedAs("Expected that " + dow2 + " is after " + current + ",  but was not").isTrue();
                }
            }
            current = dow;
        }
