...
public class HasPublicStaticValueOfMethodValidator implements ConstraintValidator<HasPublicStaticValueOfMethod, Object> {
    ...
    private static Result analyze(final Class<?> clasz, final String methodName, final Class<?> paramClass) {
        try {
            ...
            final Object value = method.invoke(clasz, (Object) null);
            if (value != null) {
                return new Result("Method '" + methodName + "' is expected to return 'true' on 'null' argument, but was: " + value + " (#4)", null);
            }
